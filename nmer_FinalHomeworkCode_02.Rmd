...
date: "`r Sys.Date()`" # This is a nice addition to get the current date whenever the html is knitted. 
output: 
...
knitr::include_graphics('https://bloximages.newyork1.vip.townnews.com/denvergazette.com/content/tncms/assets/v3/editorial/9/32/9325056a-9436-11ed-8154-bba9ef2fd03b/63c2f00b8c196.image.jpg') 

# I HAD TO CHANGE THE LOCATION OF THE PHOTO TO BE WHERE IT IS LOCATED ON MY COMPUTER IN ORDER FOR IT TO WORK (:
# Change in final code: I changed this from a file path to a link
```

------------
### Peer Commentary
I like your code to get the image to load on the knitted file. Looking at this code along with the internet/textbooks helped me add my own photo as I was having a lot of issues with it. 
--------------

# Challenges (PEER COMMENTARY THROUGHOUT THIS SECTION)
...
2. I tried using library(prettydoc) to load in the html themes for the markdown (with include=FALSE in the chunk so it wouldn't show up in the knitted html) but that returned an error when knitting because the output was being read before the library command was being read later on down in the file. I used the R Markdown Definitive Guide to format the YAML for the prettydoc themes and table of contents. #Yeah, I had to google this in order to figure out how to get the prettydoc theme to work in the YAML.
...
----------------------
### Peer Commentary
To find the value of a specific amount of calls, you can also use the dpois function. 
For example:

```{r PeerCommentary, echo=TRUE}
dpois(0,15)
```

See, you get the same answer :) 

You can also do this to find the answer for 3 calls!
--------------------------


Again this makes sense because poisson distributions tend to have right skew and having zero calls in a session would be extremely unlikely!
...
PThree
#Edit because of Peer Commentary, adding the dpois way of solving
dpois(3, 15)
```
...
------------------
### Peer Commentary
You can also just use years <- rpois(104,15) to make it a more simple code -- I never thought to do it how you did it though - it gets the same results. I like looking at your codes because they allow me to have new ideas on how to do my codes and how to get things to work. 
----------------
...
```{r Hist, echo=TRUE}
hist(years, breaks = c(0:30), probability = TRUE, #I like that you used breaks in your histogram. It makes it look nicer than mine does!
     main = "Probability of Titi Calls in 2 years", 
     xlab = "Calls", ylab = "probability", xlim = range(0:30)) #I didn't include the word range in my code. I'm wondering if that is why mine is different. Yours seems right, so I will definitely be adding the word range to my xlim. 
```

-----------------
### Peer Commentary
Okay, yeah I thought the x axis was calls. I left my y axis labeled as frequency, but I will definitely be changing the name to probability! 
------------------
